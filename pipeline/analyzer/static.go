...
type manifestApp struct {
	...
	MetaData    []util.MetaDatum  `xml:"meta-data"`
	Receivers    []util.Component  `xml:"receiver"`
	Activities    []util.Component  `xml:"activity"`
	Services    []util.Component  `xml:"service"`
}
...
func parseManifest(app *util.App) (manifest *AndroidManifest, gotIcon bool, err error, manifestJson string) {
	...
	// Convert manifest file for storage in database
	xml := strings.NewReader(string(bytes[:]))
  	json, err := xj.Convert(xml)
  	if err != nil {
  		fmt.Printf("Conversion of Manifest to JSON failed")
  		manifestJson = ""
  	}
  	manifestJson = json.String()
	...
	if len(split) != 2 {
		return manifest, false, nil, manifestJson
	}
	...
	nBytes, err := copy(matches[0], path.Join(app.AppDir(), "icon.png"))
	if err != nil || nBytes == 0 {
		fmt.Printf("Failed to rename icon %s to %s\n", matches[0], path.Join(app.AppDir(), "icon.png"))
		return manifest, false, nil, manifestJson
	}

	return manifest, true, nil, manifestJson
}

func (manifest *AndroidManifest) getPerms() []util.Permission {
	return append(manifest.Perms, manifest.Sdk23Perms...)
}
func (manifest *AndroidManifest) getComponents() []util.Component {
	return append(append(manifest.Application.Activities, manifest.Application.Receivers...), manifest.Application.Services...)
}
...
func simpleTrackers(manifestJson string) (hasFB bool, hasFirebase bool, hasGCM bool, hasGAds bool) {
	return strings.Contains(manifestJson, "com.facebook.sdk.ApplicationId"),
           strings.Contains(manifestJson, "com.google.firebase"),
           strings.Contains(manifestJson, "com.google.android.c2dm.intent"),
           strings.Contains(manifestJson, "com.google.android.gms.ads")
}
...
func simpleAnalyze(app *util.App) ([]string, error) {
	...
	// TODO: combine steps to increase speed?
