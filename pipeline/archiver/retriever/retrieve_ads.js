    ...
    while(true) {
        ...
        const res = await db.query(
            'SELECT a.id, app, store_url \
             ...
             WHERE adSupported IS NULL AND date_part(\'year\', released) > 2000 \
        ...
        try {
            await Promise.all( res.rows.map( app => {
                ...
                return retriever.app({
                    ...
                    if (appData.adSupported !== undefined) {                                
                        logger.debug(`updating ${appData.title} with ${appData.adSupported} in DB`);
                        await db.query(
                            'UPDATE playstore_apps \
                             SET adSupported = $1 \
                             WHERE id = $2',
                             [appData.adSupported, app.id]
                        );
                    } else {
